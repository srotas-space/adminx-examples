// test/src/models/config_model.rs
use actix_web::web;
use mongodb::{
...
pub async fn get_config_by_key(
    ...
    let filter = doc! { "key": key, "deleted": false };

    if let Some(mut config) = collection.find_one(filter, None).await? {
        if config.data_type == ConfigDataType::Json {
            if let Some(Value::String(ref s)) = config.data {
                if let Ok(parsed_json) = serde_json::from_str::<Value>(s) {
                    config.data = Some(parsed_json);
        ...
        let _: () = redis_conn.set_ex(&cache_key, serialized, 300).await?;
        return Ok(Some(config));
    }
